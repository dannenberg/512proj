...
//public class TCPResourceManagerPlane extends java.rmi.server.UnicastRemoteObject
public class TCPResourceManagerPlane
implements ResourceManager {
    ...
    public static void main(String args[]) {
        ...
        if (args.length == 1) {
            ...
            System.out.println("Usage: java ResImpl.TCPResourceManagerPlane [port]");
            System.exit(1);
        }
        ...
        try 
        {
            ...
            TCPResourceManagerPlane planeRM = new TCPResourceManagerPlane();
            // set up port for client connections
            int serverport = 9898; //TODO make dynamic
            ServerSocket listenSocket = new ServerSocket(serverport);
            System.err.println("PlaneRM ready!");
            while (true) {
                Socket clientSocket = listenSocket.accept();
                ConnectionRMP c = new ConnectionRMP(clientSocket, planeRM);
            }
        } 
        catch (Exception e) 
        {
    ...
    public TCPResourceManagerPlane() {
    }
    ...
    public void incrementItem(int id, String key, int change) {
        ReservableItem curObj = (ReservableItem) readData(id, key);
        Trace.info("Plane reservation cancelled: " + key);
...
class ConnectionRMP extends Thread {
    DataInputStream in;
    DataOutputStream out;
    Socket clientSocket;
    TCPResourceManagerPlane master;
    public ConnectionRMP (Socket aClientSocket, TCPResourceManagerPlane mast) {
        master = mast;
        try {
            ...
            this.start();
        } catch(IOException e) {System.out.println("ConnectionRMP:"+e.getMessage());}
    }
    public void run(){
        try {
            while(true)
